...
## ویژگی‌های جدید (v2.5.0) 🚀🚀

- **Smart Health Check**: سیستم جدید برای تشخیص و حذف کانکشن‌های مرده (Zombie Sessions) در کمتر از ۱۰ ثانیه. دیگه با قطعی‌های "وصله ولی دیتا نمیاد" خداحافظی کنید! 👋
- **Memory Optimization**: استفاده از `sync.Pool` برای کاهش مصرف رم و پرش‌های CPU (کاهش لگ در سرعت‌های بالا). 🧠

## ویژگی‌های جدید (v2.4.8) 🚀

- **Setup UI**: اضافه شدن گزینه **Connection Pool** در منوی نصب (کلاینت) برای تنظیم راحت‌تر تعداد کانکشن‌ها.
- **Default**: مقدار پیش‌فرض Connection Pool به **8** افزایش یافت (برای عبور از محدودیت سرعت).

## ویژگی‌های جدید (v2.4.7) 🚀

- **Revert**: بازگشت Frame Size به **32KB** (چون 2KB باعث فشار پردازشی و افت سرعت شد).
- **Recommendation**: برای عبور از 160Mbps، تعداد کانکشن‌ها (`connection_pool`) را در کلاینت افزایش دهید.

## ویژگی‌های جدید (v2.4.6) 🚀

- **DPI Bypass**: تغییر سایز فریم‌ها به **2KB** برای عبور از فیلترینگ هوشمند (حل مشکل افت سرعت روی 160Mbps).
- **High Performance**: حفظ بافرهای بزرگ (2MB) برای تضمین سرعت بالا.

## ویژگی‌های جدید (v2.4.5) 🚀

- **Critical Fix**: حل مشکل "Zombie Sessions" با جلوگیری از گم شدن داده‌ها در هندشیک (Backport از نسخه اصلی).
- **Stability**: پایداری بیشتر در اتصالات طولانی مدت.
